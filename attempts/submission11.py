...
def calc_absolute_angle(x1, y1, x2, y2):
    vector_1 = [x1, y1]
    vector_2 = [x2, y2]
    unit_vector_1 = vector_1 /  np.clip(np. linalg. norm(vector_1),0.00001,100000000)#np.maximum(1e-8, np. linalg. norm(vector_1))
    unit_vector_2 = vector_2 / np.clip(np. linalg. norm(vector_2),0.00001,100000000)#np.maximum(1e-8, np. linalg. norm(vector_2))
...
def behavior1(x,y,vx,vy,angle, next_check_point_id, loop, last_checkpoint_id, boost):
    ...
    dist_turn = 2400     # At what distance from checkpoint we start to turn to the next checkpoint
    max_angle = 120
    ...
    next_next_checkpoint_x, next_next_checkpoint_y = checkpoints[(next_check_point_id+1)%len(checkpoints)]
    ...
    next_checkpoint_angle_abs = calc_angle(next_checkpoint_x, next_checkpoint_y, next_next_checkpoint_x, next_next_checkpoint_y)
    turning_strength_after_checkpoint = abs((next_checkpoint_angle_abs - angle + 180) % 360 - 180)
    ...
    dist_turn_adjusted = map_value(turning_strength_after_checkpoint,
                                    0,180,dist_turn/2, dist_turn*1.5)
...
while True:
    ...
    def hunter():
        ...
        if progression_2 < progression_1:
            target = np.array([[x_1 + time_steps_ahead*vx_1], [y_1 + time_steps_ahead*vy_1]])
            to_target = target - np.array([[x2],[y2]])
        else:
            target = np.array([[x_2 + time_steps_ahead*vx_2], [y_2 + time_steps_ahead*vy_2]])
            to_target = target - np.array([[x2],[y2]])
        print("target_angle", file=sys.stderr, flush=True)
        print(to_target, file=sys.stderr, flush=True)
